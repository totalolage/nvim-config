...
local log_file_path = vim.fn.stdpath "state" .. "/autofix.log"
...
local function log_autofix(message)
...
local function resolve_code_action(client, action, bufnr, timeout_ms)
  if action.edit or action.command or not action.data or not client:supports_method("codeAction/resolve", bufnr) then
    log_autofix(string.format("resolve skipped client=%s %s", client.name, summarize_code_action(action)))
    return action
  end

  log_autofix(string.format("resolve request client=%s %s", client.name, summarize_code_action(action)))
...
local function apply_code_action(client, action, bufnr)
  log_autofix(string.format("apply start client=%s %s", client.name, summarize_code_action(action)))
  ...
  log_autofix(string.format("apply command client=%s status=skipped", client.name))
end
...
local function run_client_fix(client, bufnr)
  ...
  if not response then
    log_autofix(string.format("codeAction response client=%s status=timeout", client.name))
  ...
  if response.result == nil or response.result == vim.NIL then
    log_autofix(string.format("codeAction response client=%s status=empty result=nil", client.name))
    return true
  end
  ...
  if not action then
    log_autofix(string.format("codeAction selection client=%s status=no_fix_all_action", client.name))
    return true
  end
  ...
  if dumps_differ(selected_action_dump, resolved_action_dump) then
    log_autofix(string.format("codeAction debug client=%s stage=resolved_changed true", client.name))
  end
  ...
  if not ok then
    log_autofix(string.format("apply failure client=%s error=%s", client.name, tostring(err)))
    return false, client.name .. " fixer failed: " .. err
  end

  log_autofix(string.format("client fix complete client=%s status=success", client.name))
...
function M.autofix(bufnr)
  ...
  log_autofix(
    string.format(
      "autofix entry bufnr=%d filetype=%s total_clients=%d log_file=%s",
      bufnr,
      tostring(filetype),
      #discovered_clients,
      log_file_path
  ...
  if vim.tbl_isempty(discovered_clients) then
    log_autofix "autofix clients none"
  else
    for _, client in ipairs(discovered_clients) do
      log_autofix("autofix client " .. summarize_client(client, bufnr))
    end
  end

  if not js_fixable_filetypes[filetype] then
    log_autofix(string.format("autofix skip filetype=%s", tostring(filetype)))
    return
  end

  local fix_clients = get_fix_clients(bufnr)
  log_autofix(string.format("autofix eligible_fix_clients=%d", #fix_clients))

  for _, client in ipairs(fix_clients) do
    local ok, err = run_client_fix(client, bufnr)
    if not ok then
      notify_fix_failure(err)
      log_autofix(string.format("autofix notify_failure client=%s message=%s", client.name, err))
    end
  end
end
